...
IOManager::IOManager( ) :
  ...
  MAX_STRING_SIZE( gdata->getXmlInt("maxStringSize") ),
    ...
    screen(SDL_SetVideoMode(gdata->getXmlInt("viewWidth"), 
                            gdata->getXmlInt("viewHeight"), 0, SDL_HWSURFACE)),
...
void IOManager::printMessageCenteredAt( const string& msg, Uint32 y) const {
   ...
   if (stext) {
     ...
     throw 
     string("Couldn't allocate text surface in printMessageCenteredAt");
   }
}
...
void IOManager::printMessageValueAt(const string& msg, T value, 
   ...
   if (stext) {
     ...
     throw 
     string("Couldn't allocate text surface in printMessageValueAt");
   }
}
