...
void Viewport::trackSprite(const Drawable *obj) { 
  objectToTrack = obj; 
  objWidth = objectToTrack->getFrame()->getWidth();
  objHeight = objectToTrack->getFrame()->getHeight();
}

void Viewport::trackSprite(const Frame& fr) {
  //objectToTrack = static_cast<Drawable *>(&temp); 
  objWidth = fr.getWidth();
  objHeight = fr.getHeight();
}

void Viewport::trackSprite(const Player *player) {
  MultiframeSprite temp= player->getSprite();
  objectToTrack = static_cast<Drawable *>(&temp); 
  objWidth = objectToTrack->getFrame()->getWidth();
  objHeight = objectToTrack->getFrame()->getHeight();
}
...
void Viewport::update() {
  ...
  position[0] = (x + objWidth/2) - viewWidth/5;
